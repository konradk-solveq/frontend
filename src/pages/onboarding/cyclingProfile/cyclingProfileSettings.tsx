...
const CyclingProfileSettings: React.FC<Props> = (props: Props) => {
    ...
    const riderProfileData = useAppSelector<RiderProfile>(
    ...
    const handleGetDataBack = () => {
        // dla przycisku przywróć
        setDataSetting(memo);
    ...
    const handleGoBackWithMemeo = async () => {
        ...
        for (let i = 0; i < lists.length - 1; i++) {
            // zlicza które profile na listach zostały wybrane, poza ostatnią (mężczyna/kobieta)
            let key = lists[i];
        ...
        let countMaxies = 0;
        countProfiles.forEach(e => {
            if (e == maxVal) {
                countMaxies++;
            }
        });
        if (countMaxies > 1) {
            moreThanOneSameValue = true;
        }

        let profNum: number = -1;
        if (moreThanOneSameValue) {
            let profiles: Array<number> = [];
            countProfiles.forEach((e, i) => {
                if (e == maxVal) {
                    profiles.push(i);
                }
            });
            profNum = Math.floor(
                profiles.reduce((a, b) => a + b) / profiles.length,
            );
        } else {
            countProfiles.forEach((e, i) => {
                if (e == maxVal) {

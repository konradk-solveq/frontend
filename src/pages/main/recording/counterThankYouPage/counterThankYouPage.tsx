...
import { pointToComaString, twoDigits, simplyTimer } from '../../../../helpers/stringFoo';
...
const CounterThankYouPage: React.FC<Props> = (props: Props) => {
    ...
    const heandleGetTitleType = () => {
        if (titleType == 0) {
            ...
            if (num < 1) num = 1;
            return ' ' + num + ' ' + (num == 1 ? trans.type_1[0] : trans.type_1[1]);
        } else {
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.scroll}>
                <ScrollView>
                    <Text style={styles.title}>{userName + trans.title +
                        heandleGetTitleType()
                    }</Text>

                    {show && <AnimSvg style={styles.laurelWreath} source={laurelWreath} />}


                    <View style={styles.recorded}>
                        <View>
                            <Text style={styles.name}>{trans.distance}</Text>
                            <Text style={styles.value}>{pointToComaString(distance.toFixed(2))}
                                <Text style={styles.unit}>{' ' + trans.distanceUnit}</Text>
                            </Text>
                        </View>

                        <View >
                            <Text style={styles.name}>{trans.time}</Text>
                            <Text style={styles.value}>{simplyTimer(time)}
                                <Text style={styles.unit}>{' ' + trans.timeUnit}</Text>
                            </Text>
                        </View>
                    </View>

                    <Text style={styles.breakName}>{trans.break + '  '}
                        <Text style={styles.breakValue}>{simplyTimer(breakTime)}</Text>

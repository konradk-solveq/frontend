...
function initMap() {
    ...
    clusterPublic = new markerClusterer.MarkerClusterer({
            map,
            markers: [],
            algorithm: clusteringAlgorithm,
            renderer: clusterRenderer("pinroute_published_empty.png"),
    });
    clusterPrivate = new markerClusterer.MarkerClusterer({
            map,
            markers: [],
            algorithm: clusteringAlgorithm,
            renderer: clusterRenderer("pinroute_private_empty.png"),
    });   
    clusterPlanned = new markerClusterer.MarkerClusterer({
            map,
            markers: [],
            algorithm: clusteringAlgorithm,
            renderer: clusterRenderer("pinroute_planned_empty.png"),
        });
...
/**
* minPoints - minimum number of points needed to form a cluster
* extent - tile size in px, used to calculate the clustering radius (default radius: 40px)
* maxZoom - maximum level of zoom on which clustering takes place, 
*           zooming above that level causes all single markers to be shown
*/
const clusteringAlgorithm = new markerClusterer.SuperClusterAlgorithm({minPoints: 3, extent: 256, maxZoom: 23});

/**
* the base for rendering cluster markers, the only changing thing is the icon,
* which is passed as a parameter
*/
const renderCluster = (icon) => ({ count, position }) => {
...
const clusterRenderer = (icon) => ({render: renderCluster(icon)})
...
const setMarks = places => {
    ...
    clusterPublic.addMarkers(marks);
    clusterPrivate.addMarkers(privateMarks);
    clusterPlanned.addMarkers(plannedMarks);
}
